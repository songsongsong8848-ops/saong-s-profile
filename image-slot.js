...
(() => {
  ...
  function save() {
    ...
    if (!w) return;
    saving = true;
    const bytes = JSON.stringify(slots);
    Promise.resolve(w(STATE_FILE, bytes))
      .catch((e) => {
        console.error('[image-slot] 保存失败，图片可能不会持久化（存储文件 ' +
          (bytes.length / 1048576).toFixed(1) + 'MB，可能超出写入上限）:', e);
        try { window.dispatchEvent(new CustomEvent('image-slot-save-error', { detail: { bytes: bytes.length } })); } catch (_) {}
      })
  ...
  async function toDataUrl(file, targetW) {
    ...
    try {
      const cap = Math.min(MAX_DIM, Math.max(1, Math.round(targetW * 2.4)) || MAX_DIM);
